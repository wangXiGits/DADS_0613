...
public class SaveSportsNo8PacketUtil {
	 ...
	 * @return
	 */
	public static boolean insertNo8DataForOneWay(String patientID, String company,String deviceID,
			String stepvalue, String data_newcontent_stepcount2, String realTime, String firmware_version, String deviceType) {
...
//		boolean isValid = checkRetransmission(patientID,stepvalue,"8-1",5);
		...
		if(isValid){
			...
			result = C3P0Util.insertOrUpdateData(sql, patientID,company,deviceID,data_newcontent_stepcount2,"8-1",DateUtil.getCurrentTime(),stepvalue,realTime,firmware_version,deviceType);
			if(result)
			{
	...
	public static boolean insertNo8DataForTwoWay(String patientID, String company,String deviceID,
		...
		String sql = "insert into "+tablename+"(patientID,company,deviceID,STEP2NEWVALUE,packageNum,receiveTime,realTime) values(?,?,?,?,?,now(),?)";
	...
	public static boolean insertNo8DataForThreeWay(String patientID,String company,String device_id, String stepvalue,
			String data_newcontent_stepcount2, String locationInfoStr, String realTime, String firmware_version, String deviceType) {
...
//		boolean isValid = checkRetransmission(patientID,stepvalue,"8-3",5);
		...
		if(isValid){
			...
			C3P0Util.insertOrUpdateData(sql, patientID,company,device_id,data_newcontent_stepcount2,"8-3",DateUtil.getCurrentTime(),stepvalue,locationInfoStr,realTime,firmware_version,deviceType);
			if(result){
	...
	public static boolean insertNo8DataForFourWay(String patientID, String company,String deviceID,
		...
		String tablename = ValidateUtil.isValid(patientID)?"packagedata":"exceptiondata";
		// add by renchuanmin for PWS0009 on 20130821 end *******
		String sql = "insert into "+tablename+"(patientID,company,deviceID,STEP3NEWVALUE,packageNum,receiveTime,realTime) values(?,?,?,?,?,now(),?)";
	...
	public static boolean saveNewSportHistory(IoSession session, No8OneWayPacket packet) {
		...
		boolean ret = C3P0Util.insertOrUpdateData(sql, (String)session.getAttribute("patientId"),(String)session.getAttribute("deviceId"),
				(String)session.getAttribute("appType"),Constants.DATATYPE_STEPCOUNT,DateUtil.format(packet.getStepdate()),jsArr.toString());
		return ret;
	}
	/**
	 * 存储新协议格式-详细包
	...
	public static boolean saveNewSportDetail(IoSession session, String data, String stepdate) {
		...
		String sql = "insert into sports(phone,deviceId,apptype,dataType,realTime,datavalue,receiveTime) values(?,?,?,?,?,?,now())";
		boolean ret = C3P0Util.insertOrUpdateData(sql, (String)session.getAttribute("patientId"),(String)session.getAttribute("deviceId"),
				(String)session.getAttribute("appType"),Constants.DATATYPE_STEPDETAIL,DateUtil.format(stepdate),dataValue.toString());
		return ret;
	}
	/**
	 * 以新协议格式存储运动数据-简要包
	 ...
	 * @param stepdate
	 */
	public static boolean saveNewSportEffective(IoSession session, String data,
			String stepdate) {
		...
		String sql = "insert into sports(phone,deviceId,apptype,dataType,realTime,datavalue,receiveTime) values(?,?,?,?,?,?,now())";
		boolean ret = C3P0Util.insertOrUpdateData(sql, (String)session.getAttribute("patientId"),(String)session.getAttribute("deviceId"),
				(String)session.getAttribute("appType"),Constants.DATATYPE_STEPEFFECTIVE,DateUtil.format(stepdate),dataValue.toString());
		return ret;
	}
	/**
	 ...
	 * @return
	 */
	private static boolean checkRetransmission(String patientID, String stepvalue,String packageNum,
			int minutes) {
		String endTime = DateUtil.getCurrentTime();
		String startTime = DateUtil.addMinutes(endTime, -minutes);
		String sql = "SELECT count(*) FROM packagedata WHERE patientID='"+ patientID +"' AND packageNum='"+packageNum+"'  AND receiveTime " +
				"BETWEEN '"+startTime+"' AND '"+endTime+"' AND totalSteps='"+stepvalue+"';";
